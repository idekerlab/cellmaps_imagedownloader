...
- ``samples.csv``:
    When ``--samples`` was supplied, this is the CSV_ copy of the file passed on the command line. When the HPA workflow
    was triggered (neither ``--samples`` nor ``--cm4ai_table``) this file is generated automatically before downloads begin.

- ``samplescopy.csv``:
    A CSV_ file generated from the in-memory sample list. It is emitted whenever the tool needed to synthesize samples
    (for example when ``--cm4ai_table`` was used or when samples were built from HPA).
...
- ``unique.csv``:
    ...
    This file is only created when the optional/legacy ``--unique`` input is provided.
...
- ``proteinatlas.xml.gz``:
    A gzipped XML file containing information fetched from the `Human Protein Atlas`_. This file is only present when
    the automatic HPA workflow was executed.

- ``README.txt``:
    A short text file summarizing the tool version, description, and produced outputs. Generated for every run.

- ``task_<timestamp>_start.json`` / ``task_<timestamp>_finish.json``:
    Metadata files emitted by :mod:`cellmaps_utils.logutils` describing when the run started/finished, the CLI arguments,
